...
#'
#' @param study_name TCGA study name
#' @param da data frame containing Sample, Chromosome, Start, End, Num_Probes, Segment_Mean
#' @param grid grid of purities, ploidies over which to search for CNH+ (matrix)
#' @param k how many candidate solution to return
#' @param both whether to find CNH+ as well as CNH (default is F, i.e., CNH+ only)
#' @return saves csv results file named after the study, with CNH+, purity, ploidy for each sample
...
#' @export
analyze_TCGA_study = function(study_name, da, grid, k, both = FALSE) {
  #
  checkmate::assert(
    checkmate::check_character(study_name),
    checkmate::check_data_frame(da),
    checkmate::check_matrix(grid),
    checkmate::check_number(k)
  )
  #
  #
  snames = unique(da$Sample)
  #
  oo = res = list()
  pb = progress::progress_bar$new(total = length(snames))
  for (i in 1:length(snames)) {
    #
    pb$tick()
    #
    sample_name = snames[i]
    sa = da[da$Sample == sample_name,]
    #
    r = 2^sa$Segment_Mean
    w = sa$End - sa$Start
    cnh_plus = find_cnhplus(grid=grid, r=r, w=w, k = k, plus = TRUE)
    if (both == TRUE) {
      cnh = find_cnhplus(grid=grid, r=r, w=w, k = k, plus = FALSE)
      oo[[i]] = data.frame(sample_name, cnh, cnh_plus)
      res[[i]] = data.frame(sample_name, cnh[1,], cnh_plus[1,])  # top1
    } else {
      oo[[i]] = data.frame(sample_name, cnh_plus)
      res[[i]] = data.frame(sample_name, cnh_plus[1,])  # top1
    }
    # row.names(oo)
    #
  }
  ...
  if (both == TRUE) {
    names(res) = c('sample', 'purity', 'ploidy', 'cnh', 'purity_plus', 'ploidy_plus', 'cnh_plus')
  } else {
    names(res) = c('sample', 'purity_plus', 'ploidy_plus', 'cnh_plus')
  }
  #
  # save it
  write.csv(res, paste0(study_name, '_results.csv'), quote = F, row.names = F)
  #
  return(oo)
  #
}
